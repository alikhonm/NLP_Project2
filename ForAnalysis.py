...
def print_top_grams(unigram_probs, bigram_probs):
    ...
    for w, p in sorted(unigram_probs.items(), key=lambda x: x[1], reverse=True)[:10]:
        print(f"P({w}) = {p}")

    print("\nTop 10 Bigram Probabilities:")
    for (w1, w2), p in sorted(bigram_probs.items(), key=lambda x: x[1], reverse=True)[:10]:
        print(f"P({w2}|{w1}) = {p}")
...
def main():
    ...
    while True:
        choice = input("\n1: Reprint probabilities  2: Generate sentence  3: Apply Add-1 smoothing  "
                       "4: Calculate Perplexity on test data  5: Quit 6: Print top 10 uni|bi grams\n> ")
